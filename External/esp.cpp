...
void esp::box(ImDrawList* draw_list) {
    ...
    for (int i = 0; i <= 31; i++) {
        ...
        float dist = sqrt(pow(entity_origin.x - globals::local_player.origin.x, 2) + pow(entity_origin.y - globals::local_player.origin.y, 2)) * 0.0254f;
...
void esp::name(ImDrawList* draw_list) {
    if (!globals::local_player.pointer)
        return;
    ...
    for (int i = 0; i <= 31; i++) {
        ...
        if (feet.z > 0.01f) {
            int index = memory->read<int>(globals::enemies[i] + 0x64);
...
void esp::weapon(ImDrawList* draw_list) {
    if (!globals::local_player.pointer)
        return;
    ...
    for (int i = 0; i <= 31; i++) {
        ...
        if (feet.z > 0.01f) {
            DWORD active_weapon_handle = memory->read<DWORD>(globals::enemies[i] + offsets::m_hMyWeapons);
...
void esp::health(ImDrawList* draw_list) {
    if (!globals::local_player.pointer)
        return;
